...
class TestCreateAllFeatures:
    ...
    def test_feature_value_ranges(self, sample_df, capsys):
        ...
        assert result.loc[recent_idx, 'recency_score'] > result.loc[old_idx, 'recency_score']
    ...
    def test_creates_interaction_features(self, sample_df):
        ...
        assert (result['cvss_epss_product'] >= 0).all()
        assert result['cvss_epss_product'].max() <= 10.0  # Max CVSS * 1 (max EPSS)
        
        # Check KEV-healthcare interaction
        kev_healthcare = result[(result['kev_flag'] == 1) & (result['is_healthcare'] == 1)]
        if len(kev_healthcare) > 0:
            assert (kev_healthcare['kev_healthcare_interaction'] == 1).all()
    
    def test_creates_healthcare_flag(self, sample_df):
        """Test healthcare flag is preserved"""
        result = create_all_features(sample_df, FEATURE_COLS)
        
        assert 'is_healthcare' in result.columns
        
        # Healthcare flag should be binary
        assert result['is_healthcare'].isin([0, 1]).all()
    
    def test_creates_attack_flag(self, sample_df):
        """Test attack-related features"""
        result = create_all_features(sample_df, FEATURE_COLS)
        
        assert 'has_attack' in result.columns
        assert 'attack_technique_count' in result.columns
        
        # has_attack should be binary derived from attack_technique_count
        assert result['has_attack'].isin([0, 1]).all()
        
        # If attack_technique_count > 0, has_attack should be 1
        with_attacks = result[result['attack_technique_count'] > 0]
        if len(with_attacks) > 0:
            assert (with_attacks['has_attack'] == 1).all()
    
    def test_creates_chpl_flag(self, sample_df):
        """Test CHPL flag is preserved"""
        result = create_all_features(sample_df, FEATURE_COLS)
        
        assert 'chpl_flag' in result.columns
        
        # CHPL flag should be binary
        assert result['chpl_flag'].isin([0, 1]).all()
    ...
    def test_creates_all_expected_features(self, sample_df):
        ...
        base_features = [
            'cvss_norm', 'epss_score', 'kev_flag', 'is_healthcare',
            'attack_flag', 'attack_technique_count', 'chpl_flag'
        ]
        
        # Derived features (created by feature engineering)
        derived_features = [
            'days_since_published', 'recency_score',
            'cvss_epss_product', 'kev_healthcare_interaction',
            'has_attack'  # Binary version of attack_flag
        ]
    ...
    def test_handles_missing_epss(self):
        ...
        assert not result['cvss_epss_product'].isna().all()
    ...
    def test_handles_edge_case_old_cve(self):
        ...
        assert result['recency_score'].iloc[0] < 0.1
    ...
    def test_no_null_in_features(self, sample_df):
        ...
        feature_cols = [
            'cvss_norm', 'recency_score', 'cvss_epss_product',
            'kev_healthcare_interaction', 'has_attack', 'days_since_published'
        ]
    ...
    def test_consistent_output_for_same_input(self, sample_df):
        ...
        result1 = create_all_features(sample_df.copy(), FEATURE_COLS)
        result2 = create_all_features(sample_df.copy(), FEATURE_COLS)
    ...
    def test_integration_with_model_training(self, sample_df):
        ...
        feature_cols = [
            'cvss_norm', 'epss_score', 'kev_flag', 'recency_score',
            'cvss_epss_product', 'is_healthcare', 'has_attack'
        ]
